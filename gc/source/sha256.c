/*
 * SHA-256 — public domain reference impl (Brad Conte).  Identical to the
 * PS2 / PSP / 3DS clients so a hash computed on GameCube matches every other
 * Save Sync client byte-for-byte.
 */
...
void sha256_hex(const uint8_t hash[32], char out_hex[65]) {
    ...
    for (int i = 0; i < 32; i++) {
        out_hex[i*2]   = H[(hash[i] >> 4) & 0xF];
        out_hex[i*2+1] = H[hash[i] & 0xF];
    }
    out_hex[64] = '\0';
}
