...
# Derived from the reference vita-mcr2vmp tool with the default zero-seeded header.
...
def create_vmp(raw: bytes) -> bytes:
    ...
    for i in range(_VMP_HASH_OFFSET, _VMP_HASH_OFFSET + 0x14):
        vmp[i] = 0
    inner_hash = hashlib.sha1(inner_salt + vmp).digest()
